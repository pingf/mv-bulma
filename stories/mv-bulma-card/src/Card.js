import Icon from 'mv-bulma-icon'
import Image from 'mv-bulma-image'
  ...
  methods: {
    renderHeader(h) {
      ...
      if ($slots.header) {
        ...
        if (hasIcon) {
          const headerTitle = $slots.icon || h('mv-icon', { name: icon }, [])
    ...
    renderImg(h) {
      ...
      const img = $slots.image || [h('mv-image', { attrs: { src: image } }, [])]
